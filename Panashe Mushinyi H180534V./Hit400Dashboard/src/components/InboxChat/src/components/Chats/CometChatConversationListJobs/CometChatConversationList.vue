...
import CometChatConversationListItem from '../CometChatConversationListItem/CometChatConversationListItem.vue'
 ...
 *
 * @displayName CometChatConversationListJobs
 */
export default {
    name: 'CometChatConversationListJobs',
            ...
            handler(_, prevProps) {
                ...
                if (previousItem !== currentItem) {
                    if (Object.keys(this.item).length === 0) {
                        ...
                        if (conversationObj) {
                            let conversationKey =
                                conversationlist.indexOf(conversationObj)
                ...
                if (
                    ...
                    if (convKey > -1) {
                        ...
                        const membersCount =
                            parseInt(groupToLeave['membersCount']) - 1
                ...
                if (prevProps.messageToMarkRead !== this.messageToMarkRead) {
                    ...
                    try {
                        ...
                        this.logError(
                            'Error in converting message to conversation',
                            error
                        )
                    }
                }
    ...
    methods: {
        ...
        makeUnreadMessageCount(conversation = {}, operator) {
            try {
                ...
                let unreadMessageCount = parseInt(
                    ...
                    if (
                        ...
                        if (
                            ...
                            this.selectedConversation.unreadMessages.forEach(
                                (message) => {
                                    unreadMessageCount =
                                        this.shouldIncrementCount(message)
                                            ? ++unreadMessageCount
                                            : unreadMessageCount
                                }
                            )
        ...
        setAvatar(conversation) {
            if (
                ...
                const char = conversation.conversationWith.name
                    .charAt(0)
                    .toUpperCase()
        ...
        updateUser(user) {
            try {
                ...
                this.logError(
                    '[CometChatConversationList] updateUser error',
                    error
                )
            }
        },
        ...
        async updateGroupMemberAdded(message, options) {
            try {
                ...
                if (conversationKey > -1) {
                    ...
                    let membersCount =
                        parseInt(conversationWithObj.membersCount) + 1
                    ...
                    options &&
                    this.loggedInUser.uid === options.user.uid
                ) {
        ...
        conversationsUpdateHandler(key, item, message, options) {
            this.logInfo(
                'CometChatConversationList: conversationsUpdateHandler',
                {
                    key,
                    item,
                    message,
