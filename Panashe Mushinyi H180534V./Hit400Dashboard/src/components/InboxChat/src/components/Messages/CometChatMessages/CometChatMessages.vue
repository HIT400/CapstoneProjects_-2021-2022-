...
import CometChatMessageHeader from '../CometChatMessageHeader/CometChatMessageHeader.vue'
import CometChatLiveReactions from '../CometChatLiveReactions/CometChatLiveReactions.vue'
...
export default {
        ...
        propsForWatcher: {
            handler(_, prevProps) {
                ...
                if (prevProps.item[idKey] !== this.item[idKey]) {
                    ...
                    prevProps.composedThreadMessage !==
                    ...
                    prevProps.actionFromListener !== this.actionFromListener
                ) {
    ...
    methods: {
        ...
        showReaction(reaction) {
            ...
            if (
                !this.hasProperty(enums.LIVE_REACTIONS, reaction.data.reaction)
            ) {
                return
            }
        ...
        updateReplyCount(messages) {
            ...
            if (messageKey > -1) {
                ...
                let replyCount = messageObj.replyCount
        ...
        newMessagesArrived(newMessage) {
            ...
            CometChatEvent.triggerHandler(
                enums.EVENTS['NEW_MESSAGES_TRIGGERED'],
                {
                    unreadMessages: unreadMessages,
                }
            )
        ...
        markMessagesAsRead(scrollToBottom) {
            ...
            unreadMessages.forEach((unreadMessage) => {
                if (
                    unreadMessage.receiverType === CometChat.RECEIVER_TYPE.USER
                ) {
                    if (this.$refs.messageListRef) {
                        messageList.push(unreadMessage)
                        this.$refs.messageListRef.markMessageAsRead(
                            unreadMessage
                        )
                    }
                } else if (
        ...
        cometChatEventListeners() {
            ...
            CometChatEvent.on(
                enums.EVENTS['MESSAGE_COMPOSED'],
                ({ messages }) => this.appendMessage(messages)
            )
