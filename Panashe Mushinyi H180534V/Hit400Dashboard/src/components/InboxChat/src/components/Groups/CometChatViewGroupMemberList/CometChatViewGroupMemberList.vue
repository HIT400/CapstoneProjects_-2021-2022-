                ...
                <table :style="styles.modalTable">
                    ...
                    <thead>
                        <tr :style="styles.modalTable.tr">
                            <th :style="styles.modalTable.tr.th">
                                {{ STRINGS.NAME }}
                            </th>
                            <th
                                :style="styles.modalTable.tr.th"
                                class="viemembers__scope"
                            >
                                {{ STRINGS.SCOPE }}
                            </th>
...
import CometChatViewGroupMemberListItem from '../CometChatViewGroupMemberListItem/CometChatViewGroupMemberListItem.vue'
    ...
    methods: {
        ...
        async banMember(memberToBan) {
            try {
                ...
                const response = await CometChat.banGroupMember(
                    guid,
                    memberToBan.uid
                )

                if (response) {
                    this.emitAction('removeGroupParticipants', {
                        member: memberToBan,
                    })
                }
            } catch (error) {
        ...
        async changeScope(member, scope = 'participant') {
            try {
                ...
                if (response) {
                    ...
                    this.emitAction('updateGroupParticipants', {
                        member: updatedMember,
                    })
                }
            } catch (error) {
                this.logError(
                    'updateGroupMemberScope failed with error: ',
                    error
                )
            }
        },
